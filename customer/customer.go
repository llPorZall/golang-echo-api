...
func CustomerAPI(app *echo.Group, resource *db.Resource) {
	...
	app.POST("/customer", customerRegistor(repository))
	app.POST("/customer/login", customerLogin(repository))
	app.GET("/customer/me", customerInformation(repository), middlewares.Authorization)
	app.PATCH("/customer/password", customerChangePassword(repository), middlewares.Authorization)
}
...
func customerRegistor(repository Repository) func(c echo.Context) error {
	return func(c echo.Context) error {
		...
		if existingCustomer, _ := repository.FindOne(bson.M{"email": request.Email}); existingCustomer != nil {
		...
		customer, err := repository.Create(request)
		if err != nil {
...
func customerLogin(repository Repository) func(c echo.Context) error {
	return func(c echo.Context) error {
		...
		customer, err := repository.FindOne(bson.M{"email": request.Email})
		if err != nil {
...
func customerInformation(repository Repository) func(c echo.Context) error {
	return func(c echo.Context) error {
		customer, err := repository.FindOne(bson.M{"email": c.Request().Header.Get("email")})
		if err != nil {
			return c.JSON(http.StatusNotFound, pkg.ErrorResponse{Message: "Customer not found", StatusCode: http.StatusNotFound})
		}
...
func customerChangePassword(repository Repository) func(c echo.Context) error {
	return func(c echo.Context) error {
		...
		err = repository.Update(bson.M{"email": c.Request().Header.Get("email")}, bson.M{"$set": bson.M{"password": password}})
		if err != nil {
			return c.JSON(http.StatusNotFound, pkg.ErrorResponse{Message: "Can't update customer password", StatusCode: http.StatusNotFound})
		}
